...
router.get('/scores/:userIdx', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const userIdxDb = await Diary.find({ userIdx }, { _id: 0, userIdx: 1 }).exec();
    ...
    if (tokenUser === dbUser) {
      ...
      const monthDiary = await Diary.find(
        { userIdx },
        {
          _id: 0, yearMonth: 1, day: 1, scoreAvg: 1, inputDate: 1,
        },
      ...
      function zero(nowDay) {
        ...
        for (let b = 0; b <= thisBetween; b++) {
          const bThis = moment(thisMonDateForm).add(b, 'days').format('YYYY-MM-DD');
        ...
        if (thisWeekDb.length === 0) {
          ...
          // 저번주
          const lastMonDate = moment().subtract(nowDay + 6, 'days').format('YYYY-MM-DD'); // 2021-12-27
          // let lastSunDate = moment().subtract(nowDay, 'days').format('YYYY-MM-DD') // 2021-01-02
          const arrLastWeek = new Array(); // 저번주 1주일 배열로 만들어 나열
          for (let c = 0; c < 7; c++) {
            const bLast = moment(lastMonDate).add(c, 'days').format('YYYY-MM-DD');
          ...
          if (lastWeekDb.length === 0) {
            ...
            for (const d of thisWeekDb) {
              const thisPicDay = monthDiary.filter((pic1) => pic1.inputDate == String(d));
              const thisPic = thisPicDay[0].scoreAvg;
              arrThisScore.push(thisPic);
            }
            // console.log(arrThisScore);

            const thisPicScore = arrThisScore.reduce((sum, currVale) => sum + currVale, 0);
            ...
            for (const e of lastWeekDb) {
              const lastPicDay = monthDiary.filter((pic2) => pic2.inputDate == String(e));
            ...
            const lastPicScore = arrLastScore.reduce((sum, currVale) => sum + currVale, 0);
