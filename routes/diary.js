...
router.post('/diary', authMiddleware, async (req, res) => {
  const {
    yearMonth, day, feelScore, sleepScore, comment,
  } = req.body;
  ...
  try {
    ...
    const input = `${yearMonth.substring(0, 4)}-${yearMonth.substring(4)}-${strDay}`;
    ...
    await Diary.create({
      diaryIdx, userIdx, yearMonth, day, feelScore, sleepScore, comment, createdAt, inputDate, scoreAvg,
    });
    res.status(201).send({
      result: '수면 기록 등록 완료',
    });
  ...
  try {
    ...
    if (tokenUser === dbUser) {
      ...
      const getDiary = await Diary.find(
        { userIdx, yearMonth },
        {
          _id: 0, diaryIdx: 1, day: 1, feelScore: 1, sleepScore: 1, comment: 1,
        },
      ).sort('day');
...
router.put('/diary/:diaryIdx', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (tokenUser == dbUser) {
      ...
      await Diary.updateOne({ diaryIdx }, {
        $set: {
          feelScore, sleepScore, comment, scoreAvg, updatedAt,
        },
      });
      res.status(200).send({
        result: '기록 수정 완료',
